...
public class MethodMappingController {
    ...
    /* 2. 메소드 방식 지정 */
    // 사용자의 요청 : RequestMapping
    // 요청 URL 을 value 속성, 요청 방식을 method 속성에 지정
    @RequestMapping(value = "/menu/modify", method = RequestMethod.GET) // value에 url을 집어넣는다.
    public String modifyMenu(Model model) {
        model.addAttribute("message", "2번메소드 실행 : GET 방식만 허용하는 메뉴 수정 핸들러 메소드 동작 확인");
        return "mappingResult";
    }
    // 순서 중요 : index (-> httpRequest   <- http Response) controller -> service
    ...
    *   DELETE  -->>    @DeleteMapping */
    ...
    public String getDeleteMenu(Model model) {
        model.addAttribute("message","3번 메소드 실행 : GET 방식의 메뉴 삭제 핸들러 메소드 동작 확인");
        return "mappingResult";
    }

    @PostMapping("/menu/delete")
    public String postDeleteMenu(Model model) {
        model.addAttribute("message","3번 메소드 실행 : POST 방식의 메뉴 삭제 핸들러 메소드 동작 확인");
        return "mappingResult";
    }
}
